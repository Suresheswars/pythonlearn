...
load_dotenv()
# FILL IN BELOW:
mcp = FastMCP("_________ Knowledge Agent")

# TODO 3: Initialize Backend Components
# Hint: Create embeddings, vector store, retriever, and LLM
# FILL IN BELOW:
embeddings = OpenAIEmbeddings(model="_________-embedding-3-small")
persist_dir = "./chroma_db_mcp"

vector_store = Chroma(
    collection_name="_________",
    embedding_function=_________,
...
# FILL IN BELOW:
retriever = vector_store.as_retriever(search_kwargs={"k": _______})

# TODO 5: Create LLM instance
# Hint: Use ChatOpenAI with gpt-4o-mini model
# FILL IN BELOW:
llm = ChatOpenAI(model="_________-mini", temperature=0)
...
def learn_website(url: str) -> str:
    ...
    try:
        ...
        splitter = RecursiveCharacterTextSplitter(
            chunk_size=_______,
            chunk_overlap=_______
        )
        chunks = splitter.split_documents(docs)
        
        # TODO 9: Add chunks to vector store
        # FILL IN BELOW:
        vector_store.add_documents(_______)
...
Answer (based ONLY on the context above):"""
    ...
    # FILL IN BELOW:
    chain = (
        {"context": _______, "question": RunnablePassthrough()}
        | prompt
        | _______
        | StrOutputParser()
    )
